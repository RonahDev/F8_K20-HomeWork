...
| `defer`    | Tải song song với HTML, chạy sau khi HTML parse xong,tải hết mã Script
sau đó đợi đọc hết mã HTML thì nó mới chạy mã JS
| `async`    | Tải xong lúc nào thì chạy lúc đó, khi chạy thì chặn parsing HTML, không đợi đọc 
xong mã HTML mà nó sẽ thực thi nuôn
